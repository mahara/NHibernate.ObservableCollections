...
# 4  = Build input file path resolution failed.
# 5  = Build framework selection failed.
# 6  = dotnet build failed.
# 7  = Test input file path resolution failed.
# 8  = Test framework selection failed.
# 9  = dotnet test failed.
# 10 = Package input file path resolution failed.
# 11 = Package framework selection failed.
...
function Initialize-Arguments {
    ...
    while ($argumentIndex -lt $scriptArguments.Count) {
        ...
        if ($argument -ieq '--version') {
            $script:ARGUMENT___VERSION = Resolve-ArgumentValue -ArgumentName $argument -ArgumentValue $scriptArguments[$argumentIndex + 1]
            $argumentIndex += 2
            continue
        }

        if ($argument -ieq '--configuration') {
            $script:ARGUMENT___CONFIGURATION = Resolve-ArgumentValue -ArgumentName $argument -ArgumentValue $scriptArguments[$argumentIndex + 1]
            $argumentIndex += 2
            continue
        }

        if ($argument -ieq '--framework') {
            $script:ARGUMENT___FRAMEWORKS = Resolve-ArgumentValue -ArgumentName $argument -ArgumentValue $scriptArguments[$argumentIndex + 1]
            $argumentIndex += 2
            continue
        }
...
function Invoke-BuildPhase {
    ...
    foreach ($buildParameterItem in @(Split-ParameterList -Value $BuildParameters)) {
        $buildParameter = Split-BuildOrTestOrPackageParameter -Parameter $buildParameterItem
...
function Invoke-TestPhase {
    ...
    foreach ($testParameterItem in @(Split-ParameterList -Value $TestParameters)) {
        $testParameter = Split-BuildOrTestOrPackageParameter -Parameter $testParameterItem
...
function Invoke-PackagePhase {
    ...
    foreach ($packageParameterItem in @(Split-ParameterList -Value $PackageParameters)) {
        $packageParameter = Split-BuildOrTestOrPackageParameter -Parameter $packageParameterItem
        $packageInputFilePath = Resolve-PackageInputFilePath -InputFileName $packageParameter.InputFileName
        ...
        if (-not (Test-HasValue -Value $script:PARAMETER___FRAMEWORKS)) {
            Write-Host '--------------------------------------------------------------------------------'
            Write-Host "Packaging `"$($packageParameter.InputFileName)`" ($script:PARAMETER___VERSION | $script:PARAMETER___CONFIGURATION | project-defined frameworks)..."
            Write-Host '--------------------------------------------------------------------------------'
            ...
            $selectedFrameworks = Select-Frameworks -AvailableFrameworks $packageParameter.Frameworks -RequestedFrameworks $script:PARAMETER___FRAMEWORKS
            ...
            Write-Host '--------------------------------------------------------------------------------'
            Write-Host "Packaging `"$($packageParameter.InputFileName)`" ($script:PARAMETER___VERSION | $script:PARAMETER___CONFIGURATION | $selectedFrameworks)..."
            Write-Host '--------------------------------------------------------------------------------'
...
function Split-BuildOrTestOrPackageParameter {
    param(
        [Parameter(Mandatory = $true)]
        [string] $Parameter
    )
...
function Join-FolderPath {
    ...
    foreach ($childFolderPath in $ChildPath) {
        $folderPath = Join-Path -Path $folderPath -ChildPath $childFolderPath
    }
