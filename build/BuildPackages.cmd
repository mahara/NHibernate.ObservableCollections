...
FOR %%G IN (%BUILD_PARAMETERS%) DO (
    ...
    SET BUILD_COMMAND=dotnet build "!WORKSPACE_FOLDER_PATH!\!BUILD_PARAMETER___PROJECT_FILE_NAME!" --property:BUILD_VERSION=!BUILD_PARAMETER___VERSION! --configuration !BUILD_PARAMETER___CONFIGURATION! --property:ENABLE_SOURCE_LINK=%BUILD_PARAMETER___ENABLE_SOURCE_LINK%
...
dotnet build "%WORKSPACE_FOLDER_PATH%\tools\Explicit.NuGet.Versions\Explicit.NuGet.Versions.slnx" --configuration Release
...
FOR %%G IN (%TEST_PARAMETERS%) DO (
    FOR /F "tokens=1,2 delims=|" %%H IN (%%G) DO (
        ...
        SET TEST_PARAMETER___TARGET_FRAMEWORKS=%%I

        FOR %%J IN (!TEST_PARAMETER___TARGET_FRAMEWORKS!) DO (
