...
REM 4  = Build input file path resolution failed.
REM 5  = Build framework selection failed.
REM 6  = dotnet build failed.
REM 7  = Test input file path resolution failed.
REM 8  = Test framework selection failed.
REM 9  = dotnet test failed.
REM 10 = Package input file path resolution failed.
REM 11 = Package framework selection failed.
...
FOR %%G IN (%PACKAGE_PARAMETERS%) DO (
    FOR /F "tokens=1,2 delims=|" %%H IN (%%G) DO (
        ...
        IF NOT DEFINED PARAMETER___FRAMEWORKS (
            ...
            SET PACK_COMMAND=dotnet pack "!PACKAGE_PARAMETER___INPUT_FILE_PATH!" --property:BUILD_VERSION=!PARAMETER___VERSION! --configuration !PARAMETER___CONFIGURATION! --no-build

            REM ECHO !PACK_COMMAND!
            !PACK_COMMAND! || EXIT /B 12
        ) ELSE (
            CALL :SELECT_FRAMEWORKS "!PACKAGE_PARAMETER___FRAMEWORK!" "!PARAMETER___FRAMEWORKS!" PACKAGE_PARAMETER___FRAMEWORKS
            IF ERRORLEVEL 1 EXIT /B 11

            ECHO --------------------------------------------------------------------------------
            ECHO Packaging "!PACKAGE_PARAMETER___INPUT_FILE_NAME!" ^(!PARAMETER___VERSION! ^| !PARAMETER___CONFIGURATION! ^| !PACKAGE_PARAMETER___FRAMEWORKS!^)^.^.^.
            ECHO --------------------------------------------------------------------------------

            SET PACK_COMMAND=dotnet pack "!PACKAGE_PARAMETER___INPUT_FILE_PATH!" --property:BUILD_VERSION=!PARAMETER___VERSION! --configuration !PARAMETER___CONFIGURATION! --property:ENABLE_SOURCE_LINK=!PARAMETER___ENABLE_SOURCE_LINK! --property:GeneratePackageOnBuild=false
