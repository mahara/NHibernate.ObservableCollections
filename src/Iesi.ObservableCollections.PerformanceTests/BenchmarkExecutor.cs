...
public class BenchmarkExecutor
{
        ...
        List<BenchmarkTypePlan> BenchmarkTypePlans)
    {
        ...
        public int BenchmarkCaseCount => BenchmarkTypePlans.Sum(static p => p.BenchmarkCaseCount);
    }
    ...
    {
        Type Type = type;
        List<Type> AllTypes = [.. Type.Assembly.GetTypes().OrderBy(static t => t.FullName, Type_FullName_StringComparer)];
        List<Type> AllBenchmarkTypes = [.. AllTypes.Where(IsBenchmarkType).OrderBy(static t => t.FullName, Type_FullName_StringComparer)];
        List<RuntimeJobDefinition> BaseRuntimeJobDefinitions = CreateRuntimeJobDefinitions(BaseRuntimes, baseJob);
        Dictionary<string, RuntimeJobDefinition> BaseRuntimeJobDefinitionsByRuntimeTfm = BaseRuntimeJobDefinitions.ToDictionary(static rj => rj.RuntimeTfm, StringComparer.Ordinal);
        ...
        string[] args)
    {
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            if (arg != "--runtime-job" &&
                !arg.StartsWith("--runtime-job=", StringComparison.Ordinal))
            {
                continue;
            }

            // --runtime-job=net10.0
            // --runtime-job="net10.0;net48"
        ...
        foreach (var runtimeTfm in values)
        {
            ...
            {
                throw new ArgumentException($"Unknown runtime TFM: '{runtimeTfm}'. Runtime must exactly match .NET TFM (Target Framework Moniker).");
            }
...
#pragma warning restore CA1859 // Use concrete types when possible for improved performance
        ...
        List<RuntimeJobDefinition> runtimeJobDefinitions)
    {
        ...
        {
            return [.. baseRuntimeJobDefinitions.Select(static r => r.JobFactory())];
        }

        return [.. runtimeJobDefinitions.Select(static r => r.JobFactory())];
    }
        ...
        List<BenchmarkTypePlan> benchmarkTypePlans)
    {
        return
        [
            .. benchmarkTypePlans.GroupBy(static p => p.BenchmarkExecutionGroupKey)
                                 .OrderBy(static g => g.Key.EffectiveFeatures)
                                 .ThenBy(static g => g.Key.HasMemoryDiagnoserAttribute)
                                 .ThenBy(static g => g.Key.HasDisassemblyDiagnoserAttribute)
                                 .Select(static g =>
                                 {
                                     var benchmarkTypePlansInGroup =
                                         g.OrderBy(static p => p.BenchmarkType.FullName, Type_FullName_StringComparer)
                                          .ToList();
        ...
        List<BenchmarkExecutionGroup> benchmarkExecutionGroups)
    {
        ...
        var baseFeatureValues = Enum.GetValues<BenchmarkFeatures>()
                                    ...
                                    .ToList().AsReadOnly();
#else
        var baseFeatureValues = Array.AsReadOnly(((BenchmarkFeatures[]) Enum.GetValues(typeof(BenchmarkFeatures))).Where(static f => f != BenchmarkFeatures.None).ToArray());
#endif
        ...
        var runtimesSummary = string.Join(", ", baseJobs.Select(static job => $"{job.Environment.Runtime} ({job.Environment.Runtime!.MsBuildMoniker})"));
        var baseFeaturesSummary = baseFeatures.ToString();
        var selectedBenchmarkTypeCount = benchmarkExecutionGroups.Sum(static g => g.BenchmarkTypeCount);
        var selectedBenchmarkCaseCount = benchmarkExecutionGroups.Sum(static g => g.BenchmarkTypePlans.Sum(static p => p.BenchmarkCaseCount));
        ...
        Console.WriteLine($"  Groups.BenchmarkTypes.Total:    {benchmarkExecutionGroups.Sum(static g => g.BenchmarkTypeCount),6}");
        Console.WriteLine($"  Groups.BenchmarkCases.Total:    {benchmarkExecutionGroups.Sum(static g => g.BenchmarkCaseCount),6}");
        ...
        foreach (var benchmarkExecutionGroup in benchmarkExecutionGroups)
        {
            ...
            var hasConfigAttribute = benchmarkExecutionGroup.BenchmarkTypePlans.Any(static p => HasConfigAttribute(p.BenchmarkType));
            ...
            Console.WriteLine($"    Exporters.Effective:    {(exporterDefinitions.Count > 0 ? string.Join(", ", exporterDefinitions.Select(static e => e.Name)) : "-")}");
        ...
        foreach (var benchmarkExecutionGroup in benchmarkExecutionGroups)
        {
            var benchmarkTypes =
                benchmarkExecutionGroup.BenchmarkTypePlans
                                       .Select(static p => p.BenchmarkType)
                                       .ToArray();
    ...
    {
        return benchmarkType.GetMethods().Any(static m => m.HasAttribute<BenchmarkAttribute>());
    }
